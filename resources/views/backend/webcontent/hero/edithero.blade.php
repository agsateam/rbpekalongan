...
@section('content')
    ...
    <div class="flex flex-col md:flex-row justify-between">
        <h4 class="text-2xl md:text-3xl font-bold mb-5">Edit Hero</h4>
    ...
    <div class="">
        <form class="p-4 md:p-5" action="{{ route('webcontent.hero.update', $hero->id) }}" method="POST"
            ...
            <div class="flex flex-wrap items-center justify-center w-full mb-4 gap-4">
                @for ($i = 1; $i <= 6; $i++)
                    ...
                    <label for="{{ $inputId }}"
                        ...
                        <div class="flex flex-col items-center justify-center pt-5 pb-6 {{ $hero->$foto ? 'hidden' : '' }}"
                            id="{{ $dropzoneId }}">
                        ...
                        <img id="{{ $previewId }}" src="{{ $hero->$foto }}" alt="Preview"
                            class="{{ $hero->$foto ? '' : 'hidden' }} w-full h-full object-cover rounded-lg" />
